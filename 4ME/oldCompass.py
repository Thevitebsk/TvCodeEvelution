...
def Compass(x:str)->None:
 ...
 def phar(x)->list:
  ...
  while len(x)>p:
   ...
   po=0  ;  p+=1
  if"main"in funcs:
   x.append(funcs["main"]) ; x.pop(x.index("main"))
        ...
        try:
         ...
         while len(cont)>contp:
          if cont[contp]in var:
           print(var[cont[contp]]);contp+=1
          else:
           print(cont[contp]);contp+=1
        except IndexError:
...
def CPEC():
 ...
 print(f"Compass v {VER} {LUD} by Gaham (Thevitebsk)","="*55,
       "to exit this seesion, type \"$exit\""
       "tho there's no manual for CPEC or this version of Compass, so...",sep="\n")
 code=[]
 while 1:
  code.append(input(">> "))
  if "$clear" in code:
